...
public class MainActivity extends AppCompatActivity {
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()) {
            case R.id.opCamera:
                dispatchTakePictureIntent(); // Chama o método para iniciar a captura de foto ao selecionar a opção da câmera
                return true;
            default:
    ...
    private void dispatchTakePictureIntent() {
        File f = null;
        try {
            f = createImageFile(); // Cria um arquivo de imagem
        } catch (IOException e) {
        ...
        if(f != null) {
            Uri fUri = FileProvider.getUriForFile(MainActivity.this, "mendes.botecchia.dara.galeria.fileprovider", f);
            Intent i = new Intent(MediaStore.ACTION_IMAGE_CAPTURE); // Cria uma Intent para capturar a foto
            i.putExtra(MediaStore.EXTRA_OUTPUT, fUri); // Define o caminho do arquivo de saída da foto
            startActivityForResult(i, RESULT_TAKE_PICTURE); // Inicia a atividade de captura de foto
        }
    }
